...
function [B, A, w_c, M] = But_FTI_FAZA1A(w_p,w_s,Delta_p,Delta_s,Ts)
...
%
	w_p = 2*tan(w_p/2)/Ts ;%Compute Omega_p.
	w_s = 2*tan(w_s/2)/Ts ;%Compute Omega_s.
	FN = 1-Delta_p ; % M_p. 
	FN = FN*FN ; 
	FN = (1-FN)/FN ; 
